...
app.post('/api/generate-facts', async (request, response) => {
  ...
  try {
    const facts = await generateFactsWithOpenAI({
      topic,
      language,
      lengthMode,
      count,
    ...
    response.json({ facts, source: 'openai' });
  } catch (error) {
...
async function generateFactsWithOpenAI({
  topic,
  language,
  lengthMode,
  count,
      ...
      messages: [
        {
          role: 'system',
          content:
            'You create concise educational notification text. Respond only with JSON: {"facts":[{"title":"...","body":"..."}]}.',
        },
        {
          role: 'user',
          content: [
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI HTTP ${response.status}: ${details.slice(0, 300)}`);
  }
  ...
  if (!Array.isArray(parsed.facts)) {
    throw new Error('OpenAI response JSON did not include facts array');
  }
